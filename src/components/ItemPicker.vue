<template>
    <div class="fixed inset-0 bg-black bg-opacity-40 overflow-y-auto h-screen w-full">
        <div class="relative top-44 mx-auto p-5 border border-gray-800 shadow-lg rounded-md " style="background-color:#1a1f1f;width:800px">
            <div class="flex justify-end cursor-pointer text-gray-200" @click="$emit('close')"> X </div>
            <div class="mt-3 max-h-96 overflow-auto">
                <div v-for="item in (items_map[selected_item]||[])" :key="item.name" class="mb-3 hover:bg-black rounded-xl p-3 cursor-pointer" @click="$emit('select-item',item)">
